...
function setupMessaging() {
    ...
    // Set up observer to re-render messages when the page becomes active
...
async function renderMessages() {
    ...
    try {
        const reports = JSON.parse(localStorage.getItem('gbv_reports_log')) || [];
        ...
        // Fetch messages for each report from the server
        let allMessages = [];
        ...
        for (const report of reports) {
            try {
                ...
                console.error(`Failed to fetch messages for report ${report.id}:`, err);
            }
        }
        ...
        list.innerHTML = allMessages.map(msg => `
            <div class="card" style="margin-bottom:12px; border-left:4px solid var(--purple);">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:8px;">
                    <div>
                        <strong style="color:var(--purple);">${escapeHtml(msg.sender)}</strong>
                        <span style="color:var(--text-muted); font-size:12px; margin-left:8px;">${msg.senderRole === 'admin' ? '🏛️ Authority' : '👤 You'}</span>
                    </div>
                    <span style="font-size:12px; color:var(--text-muted);">${new Date(msg.timestamp).toLocaleString()}</span>
                </div>
                <p style="margin-top:10px; font-size:14px; color:var(--text); line-height:1.6;">${escapeHtml(msg.message)}</p>
            </div>
        `).join('');
        
    } catch (err) {
        console.error('Error rendering messages:', err);
        list.innerHTML = '<p style="color:var(--text-muted);">Error loading messages.</p>';
    }
}
